...
def get_region_details(region: str | int):
    ...
    if isinstance(region, str):
        cursor.execute('SELECT id, ip, port, active FROM regions WHERE region = ?', (region,))
    else:
        cursor.execute('SELECT region, ip, port, active FROM regions WHERE id = ?', (region,))
...
def get_replication_dest(region_id: int, total_regions: int):
    ...
    if active == 1:
        ...
        temp_replication_dest_id = cursor.fetchone()
        if not temp_replication_dest_id or temp_replication_dest_id[0] == 0:
            conn.close()
            return '', 0
        temp_replication_dest_id = temp_replication_dest_id[0]
...
def get_temp_replication_dest(region_id: int):
    conn = sqlite3.connect('master.db')
    cursor = conn.cursor()
    cursor.execute('SELECT temp_replication_dest_id FROM regions WHERE id = ?', (region_id,))
    temp_replication_dest_id = cursor.fetchone()
    if not temp_replication_dest_id or temp_replication_dest_id[0] == 0:
        ...
        return '', 0
    temp_replication_dest_id = temp_replication_dest_id[0]
    print(temp_replication_dest_id)
    cursor.execute('SELECT ip, port, active FROM regions WHERE id = ?', (temp_replication_dest_id,))
    ip, port, active = cursor.fetchone()
    conn.close()
    return f'http://{ip}:{port}', active

def update_temp_replication_dest(region_id: int, temp_replication_dest_id: int):
    conn = sqlite3.connect('master.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE regions SET temp_replication_dest_id = ? WHERE id = ?', (temp_replication_dest_id, region_id))
    conn.commit()
    conn.close()
...
def remove_region(region):
    ...
    if prev_reg_active == 0:
        ...
        prev_reg_id = cursor.fetchone()
        if prev_reg_id:
            prev_reg_id = prev_reg_id[0]
        else:
            conn.close()
            return 0, 0
    ...
    if  all_active_nodes:
        for node in all_active_nodes:
            ...
            if all_active_nodes[0][0] != curr_reg_id:
